...
export default function EmployeeProfile() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', width: '100%', boxSizing: 'border-box' }}>
      ...
      {/* Header Info Banner */}
      <div style={{ background: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.02)', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div style={{ background: '#eff6ff', padding: '10px', borderRadius: '10px', color: '#2563eb' }}>
          <Layers size={22} />
        </div>
        <div>
          <h2 style={{ margin: '0 0 2px 0', fontSize: '16px', fontWeight: '700', color: '#0f172a' }}>Team's Profile Hierarchy</h2>
          <p style={{ margin: 0, fontSize: '12px', color: '#64748b' }}>View organizational structure, leadership, departments, and assigned team members.</p>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', width: '100%', flexWrap: 'wrap' }}>
              {clients.map(node => (
                <OrgCardViewOnly key={node.id} node={node} />
              ))}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            ...
            <div style={{ display: 'flex', gap: '30px', justifyContent: 'center', width: '100%', flexWrap: 'wrap' }}>
              {teamLeads.length > 0 ? (
                teamLeads.map(tl => {
                  ...
                  return (
                    <div key={tl.id} style={{ background: '#f8fafc', border: '2px solid #64748b', borderRadius: '14px', padding: '18px', flex: '1 1 340px', maxWidth: '450px', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 6px 12px rgba(0,0,0,0.04)', boxSizing: 'border-box' }}>
                      ...
                      <span style={{ background: '#eff6ff', color: '#2563eb', padding: '2px 10px', borderRadius: '6px', fontSize: '10px', fontWeight: '800', margin: '6px 0 14px 0' }}>{tl.department}</span>
                ...
                <div style={{ padding: '10px', color: '#94a3b8', fontSize: '12px' }}>No Team Leads found.</div>
              )}
            </div>
...
function OrgCardViewOnly({ node }) {
  return (
    <div style={{ background: '#f8fafc', border: '2px solid #cbd5e1', borderRadius: '12px', padding: '16px', width: '200px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
      ...
      <span style={{ background: '#eff6ff', color: '#2563eb', padding: '2px 8px', borderRadius: '6px', fontSize: '10px', fontWeight: '700' }}>{node.department}</span>
    </div>
  );
}